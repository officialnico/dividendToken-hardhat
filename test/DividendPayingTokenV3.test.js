...
contract("DividendPayingToken", function (accounts) {
    ...
    describe("when the contract owner tries to mint tokens", function () {
      ...
      describe("when the recipient is not the zero address", function () {
        it("mint tokens to the recipient", async function () {
          ...
          (
            await this.token.accumulativeDividendOf(tokenHolder1)
          ).should.be.bignumber.equal(ether("0"));
          (
            await this.token.withdrawableDividendOf(tokenHolder1)
          ).should.be.bignumber.equal(ether("0"));
          (
            await this.token.withdrawnDividendOf(tokenHolder1)
          ).should.be.bignumber.equal(ether("0"));
        });
      });
    });
    ...
    describe("when anyone tries to pay and distribute dividends", function () {
      ...
      describe("when paying 0 ether", function () {
        it("should succeed but nothing happens", async function () {
          ...
          await this.token.distributeDividends({
            from: anyone,
            value: ether("0"),
          });
      ...
      describe("when the total supply is not 0", function () {
        it("should pay and distribute dividends to token holders", async function () {
          ...
          (
            await this.token.accumulativeDividendOf(tokenHolder1)
          ).should.be.bignumber.equal(ether("0.25"));
          (
            await this.token.withdrawableDividendOf(tokenHolder1)
          ).should.be.bignumber.equal(ether("0.25"));
          (await this.token.dividendOf(tokenHolder1)).should.be.bignumber.equal(
            ether("0.25")
          );
          (
            await this.token.withdrawnDividendOf(tokenHolder1)
          ...
          (
            await this.token.accumulativeDividendOf(tokenHolder2)
          ).should.be.bignumber.equal(ether("0.75"));
          (
            await this.token.withdrawableDividendOf(tokenHolder2)
          ).should.be.bignumber.equal(ether("0.75"));
          (await this.token.dividendOf(tokenHolder2)).should.be.bignumber.equal(
            ether("0.75")
          );
          (
            await this.token.withdrawnDividendOf(tokenHolder2)
          ).should.be.bignumber.equal(ether("0"));
        });
      });
    });
    ...
    describe("when anyone tries to pay and distribute dividends by sending ether to the contract", function () {
      ...
      describe("when the total supply is not 0", function () {
        it("should pay and distribute dividends to token holders", async function () {
          ...
          const { logs } = await this.token.sendTransaction({
            from: anyone,
            value: ether("1"),
          });
          await expectEvent.inLogs(logs, "DividendsDistributed", {
  ...
  describe("transfer", function () {
    ...
    describe("when the recipient is the zero address", function () {
      it("reverts", async function () {
        await shouldFail.reverting(
          this.token.transfer(ZERO_ADDRESS, ether("0.5"), {
            from: tokenHolder1,
          })
        );
      });
    ...
    describe("when the recipient is not the zero address", function () {
      describe("when the sender does not have enough balance", function () {
        it("reverts", async function () {
          await shouldFail.reverting(
            this.token.transfer(tokenHolder2, ether("2"), {
              from: tokenHolder1,
            })
          );
        });
      ...
      describe("when the sender has enough balance", function () {
        it("transfers the requested amount", async function () {
          await this.token.transfer(tokenHolder2, ether("0.25"), {
            from: tokenHolder1,
          });
          ...
          (await this.token.balanceOf(tokenHolder2)).should.be.bignumber.equal(
            ether("0.25")
          );
        });
  ...
  describe("transfer from", function () {
    ...
    describe("when the recipient is not the zero address", function () {
      describe("when the spender has enough approved balance", function () {
        beforeEach(async function () {
          await this.token.approve(spender, approveAmount, {
            from: tokenHolder1,
          });
        });
        ...
        describe("when the initial holder has enough balance", function () {
          ...
          it("transfers the requested amount", async function () {
            (
              await this.token.balanceOf(tokenHolder1)
            ).should.be.bignumber.equal(mintAmount.sub(transferAmount));
            (
              await this.token.balanceOf(tokenHolder2)
            ).should.be.bignumber.equal(transferAmount);
          });

          it("decreases the spender allowance", async function () {
            (
              await this.token.allowance(tokenHolder1, spender)
            ).should.be.bignumber.equal(approveAmount.sub(transferAmount));
          });
        ...
        describe("when the initial holder does not have enough balance", function () {
          ...
          beforeEach(async function () {
            await this.token.approve(spender, _approveAmount, {
              from: tokenHolder1,
            });
          });
    ...
    describe("when the recipient is the zero address", function () {
      ...
      it("reverts", async function () {
        await shouldFail.reverting(
          this.token.transferFrom(tokenHolder1, ZERO_ADDRESS, transferAmount, {
            from: spender,
          })
        );
      });
  ...
  describe("withdrawDividend", function () {
    it("should be able to withdraw dividend", async function () {
      ...
      const receipt = await this.token.withdrawDividend({
        from: tokenHolder1,
        gasPrice: gasPrice,
      });
  ...
  describe("keep dividends unchanged in several cases", function () {
    ...
    it("should keep dividends unchanged after transferring tokens", async function () {
      ...
      await this.token.transfer(tokenHolder2, ether("1"), {
        from: tokenHolder1,
      });
    ...
    it("should keep dividends unchanged after transferFrom", async function () {
      ...
      await this.token.approve(tokenHolder3, ether("1"), {
        from: tokenHolder1,
      });
      await this.token.transferFrom(tokenHolder1, tokenHolder2, ether("1"), {
        from: tokenHolder3,
      });
    ...
    it("should correctly distribute dividends after transferring tokens", async function () {
      ...
      (
        await this.token.accumulativeDividendOf(tokenHolder1)
      ).should.be.bignumber.equal(ether("12"));
      (
        await this.token.withdrawableDividendOf(tokenHolder1)
      ).should.be.bignumber.equal(ether("12"));
      (
      ...
      (
        await this.token.accumulativeDividendOf(tokenHolder2)
      ).should.be.bignumber.equal(ether("43"));
      (
        await this.token.withdrawableDividendOf(tokenHolder2)
      ).should.be.bignumber.equal(ether("43"));
      (
  ...
  describe("end-to-end test", function () {
    it("should pass end-to-end test", async function () {
      ...
      // transfer
      await this.token.transfer(tokenHolder2, ether("2"), {
        from: tokenHolder1,
      });
      (await this.token.balanceOf(tokenHolder1)).should.be.bignumber.equal(
        ether("0")
      ...
      receipt = await this.token.withdrawDividend({
        from: tokenHolder1,
        gasPrice: gasPrice,
      });
      ...
      balanceAfter.should.be.bignumber.equal(
        balanceBefore.add(ether("10")).sub(fee)
      );
      (
        await this.token.accumulativeDividendOf(tokenHolder1)
      ).should.be.bignumber.equal(ether("10"));
      ...
      (
        await this.token.accumulativeDividendOf(tokenHolder1)
      ).should.be.bignumber.equal(ether("16"));
      (
        await this.token.withdrawableDividendOf(tokenHolder1)
      ).should.be.bignumber.equal(ether("6"));
      (
        await this.token.withdrawnDividendOf(tokenHolder1)
      ).should.be.bignumber.equal(ether("10"));
      (
        await this.token.accumulativeDividendOf(tokenHolder2)
      ).should.be.bignumber.equal(ether("14"));
      (
        await this.token.withdrawableDividendOf(tokenHolder2)
      ).should.be.bignumber.equal(ether("14"));
      (
      ...
      await this.token.transfer(tokenHolder3, ether("2"), {
        from: tokenHolder2,
      });
      ...
      await this.token.transfer(tokenHolder1, ether("2"), {
        from: tokenHolder2,
      });

      // 5 2 3

      await this.token.transfer(tokenHolder3, ether("5"), {
        from: tokenHolder1,
      });
      ...
      await this.token.transfer(tokenHolder1, ether("3"), {
        from: tokenHolder2,
      });
      ...
      (await this.token.balanceOf(tokenHolder3)).should.be.bignumber.equal(
        ether("6")
      );
      ...
      (
        await this.token.accumulativeDividendOf(tokenHolder1)
      ).should.be.bignumber.equal(ether("19"));
      (
        await this.token.withdrawableDividendOf(tokenHolder1)
      ).should.be.bignumber.equal(ether("9"));
      (
        await this.token.withdrawnDividendOf(tokenHolder1)
      ).should.be.bignumber.equal(ether("10"));
      (
        await this.token.accumulativeDividendOf(tokenHolder2)
      ).should.be.bignumber.equal(ether("15"));
      (
        await this.token.withdrawableDividendOf(tokenHolder2)
      ).should.be.bignumber.equal(ether("15"));
      (
      ...
      (
        await this.token.accumulativeDividendOf(tokenHolder3)
      ).should.be.bignumber.equal(ether("6"));
      (
        await this.token.withdrawableDividendOf(tokenHolder3)
      ).should.be.bignumber.equal(ether("6"));
      (
        await this.token.withdrawnDividendOf(tokenHolder3)
      ).should.be.bignumber.equal(ether("0"));
      ...
      fee = gasPrice.mul(new BN(receipt.receipt.gasUsed));
      balanceAfter.should.be.bignumber.equal(
        balanceBefore.add(ether("9")).sub(fee)
      );
      (
        await this.token.accumulativeDividendOf(tokenHolder1)
      ).should.be.bignumber.equal(ether("19"));
      ...
      fee = gasPrice.mul(new BN(receipt.receipt.gasUsed));
      balanceAfter.should.be.bignumber.equal(
        balanceBefore.add(ether("15")).sub(fee)
      );
      (
        await this.token.accumulativeDividendOf(tokenHolder2)
      ).should.be.bignumber.equal(ether("15"));
      ...
      fee = gasPrice.mul(new BN(receipt.receipt.gasUsed));
      balanceAfter.should.be.bignumber.equal(
        balanceBefore.add(ether("6")).sub(fee)
      );
      (
        await this.token.accumulativeDividendOf(tokenHolder3)
      ).should.be.bignumber.equal(ether("6"));
